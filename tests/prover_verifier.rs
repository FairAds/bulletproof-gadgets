...
#[test]
fn test_fake_file() {
    let instance_file = "/Users/constanzaperez/Desktop/Proyectos/fairads-mvp/src/adserver2/bulletproof-gadgets/example.inst";
    let witness_file = "/Users/constanzaperez/Desktop/Proyectos/fairads-mvp/src/adserver2/bulletproof-gadgets/example.wtns";
    let gadget_file = "/Users/constanzaperez/Desktop/Proyectos/fairads-mvp/src/adserver2/bulletproof-gadgets/example.gadgets";

    let (coms, proof) = c_prove(instance_file, witness_file, gadget_file);
    let proof_ptr = unsafe { CStr::from_ptr(coms) };
    let str_slice: &str = proof_ptr.to_str().unwrap();
    let str_buf: String = str_slice.to_owned();  // if necessary

    assert_eq!("hola", str_buf);
    ...
    // }
    // let coms, proof = c_prove(&mut bufreader_instance, &mut bufreader_witness, &mut bufreader_gadgets);
    // assert_eq("h", coms);
