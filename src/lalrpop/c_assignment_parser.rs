...
impl Assignments {
    ...
    pub fn parse_wtns(
        ...
        for line in BufReader::new(file).lines() {
            ...
            for (index, com) in commitment.1.iter().enumerate() {
                let coms_bytes = &format_com("C", &name[1..name.len()], &index, com);
                let coms_line =  String::from_utf8(coms_bytes.to_vec()).to_owned();
                let coms_string = format!("{:?}", coms_line);
                coms.push_str(&coms_string.to_owned())
            }
        }
        return coms;
    }
    ...
    pub fn parse_derived_wtns(
        ...
        gadget: usize,
        subroutine: usize,
        coms_string: &mut String
    ) -> String {
        for (index, com) in coms.iter().enumerate() {
            let identifier = format!("{}-{}", gadget.to_string(), subroutine);
            let coms_bytes = &format_com("D", &identifier, &index, com);
            let coms_line =  String::from_utf8(coms_bytes.to_vec()).to_owned();
            let coms_line_string = format!("{:?}", coms_line);

            coms_string.push_str(&coms_line_string.to_owned())
        }

        return coms_string.to_string();
    }
}
