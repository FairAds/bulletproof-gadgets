...
use gadget::Gadget;
use merkle_tree::merkle_tree_gadget::{Pattern, Pattern::*};
use mimc_hash::mimc_hash_gadget::MimcHash256;
use std::fmt;
use mimc_hash::mimc::mimc_hash_sponge;
use conversions::{scalar_to_be, scalars_to_lc, scalar_to_hex, scalar_to_bytes, bytes_to_hex_strs};
...
impl Gadget for MerkleRootHash {
    fn preprocess(&self, _: &Vec<Scalar>) -> Vec<Scalar> {
        ...
        println!("self.instance_vars = {:?}", self.instance_vars);
        println!("self.witness_vars = {:?}", self.witness_vars);
        println!("self.witness_vars = {:?}", self.pattern.to_string());
        Vec::new()
    }
    ...
    fn assemble(
        ...
        cs.constrain(LinearCombination::default());
    }
}
...
mod tests {
    ...
    fn test_merkle_root_hash_gadget_0() {
        ...
        let gadget = MerkleRootHash::new(root.into(), Vec::new(), witness_scalars, pattern);
        ...
        let derived_vars: Vec<Variable> = verifier_commit(&mut verifier, Vec::new());
