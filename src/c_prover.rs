...
use lalrpop::ast::*;
use lalrpop::c_assignment_parser::*;
...
fn generate_proof(filename_instance:  &'static str, filename_witness:  &'static str, filename_gadgets:  &'static str) -> String {
    // ---------- CREATE PROVER ----------
    let mut transcript = Transcript::new(b"prover");
    ...
    // ---------- PARSE .inst FILE ----------
    assignments.parse_inst(filename_instance.to_string()).expect("unable not read .inst file");

    // ---------- PARSE .wtns FILE, WRITE .coms FILE ----------
    let mut coms_file = assignments.parse_wtns(filename_witness.to_string(), &mut prover);
    ...
    // ---------- GADGETS ----------
    let file = File::open(filename_gadgets).expect("file not found!");
    ...
    let proof_unwrap = prover.prove(&bp_gens).unwrap();
    let proof_bytes = &proof_unwrap.to_bytes();
    let proof_str = str::from_utf8(proof_bytes).unwrap();
    let proof = proof_str.to_string();
...
#[no_mangle]
pub extern fn c_prove(filename_instance:  &'static str, filename_witness:  &'static str, filename_gadgets:  &'static str) -> *mut c_char {
    let proof = generate_proof(filename_instance, filename_witness, filename_gadgets);
    return CString::new(proof).unwrap().into_raw()
}
...
fn hash_witness(
    ...
    assignments.parse_derived_wtns(hash_commitments.clone(), index, subroutine, coms_file);
...
fn bounds_check_gadget(
    ...
    assignments.cache_derived_wtns(derived_wtns);
    assignments.parse_derived_wtns(derived_coms, index, 0, coms_file);
}
